...
//zbior funkcji pomocnych w działaniu programu

void Odwrocenie(unsigned int *tablica, int dlugosc)										//odwrocenie tablicy
{
	for(int i = 0; i < (dlugosc)/2; i++) std::swap(tablica[i], tablica[(dlugosc-1)-i]);
}

bool Czy_rosnaco(unsigned int *tablica, int dlugosc)									//sprawdzenie czy tablica jest posortowana rosnaco
{
